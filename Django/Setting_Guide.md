...
- 프로젝트 : 앱의 집합으로 URL설정 및 프로젝트의 `전반적인 설정`을 담당한다.
- 어플리케이션 : 독립적인 `각각의 기능`들을 의미한다.
...
기본적으로 Django 프레임워크에서 정보처리방식은 다음과 같다. 클라이언트(프론트엔드)가 Django 서버(백엔드)에 `url형태`로 `요청(request)`을 보내게 되며, 서버는 요청을 받아 서버 안의 `views 모듈`을 통해 `templates 모듈의 데이터(JSON, html...)`를 가져와 클라이언트에게 `응답(responses)`해주게 된다.

![](../img/django_%EC%9D%91%EB%8B%B5%2C%EC%9A%94%EC%B2%AD.jpg)
...
![](../img/django_templates.png)

<br>
<br>

![](../img/django_%EC%B6%9C%EB%A0%A5.png)
